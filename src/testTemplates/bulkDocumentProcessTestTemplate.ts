...
export class BulkDocumentProcessTestTemplate {
	 ...
	 */
	public dataCreation: (ndocuments: number, nlines: number, connection: SalesforceConnection) => Promise<void>;
...
export namespace BulkDocumentProcess {
	 ...
	 */
	export const performTestFlow = async (dpTestTemplate: BulkDocumentProcessTestTemplate) => {
...
const gatherResults = async (
	...
	try {
		await dpTestTemplate.dataCreation(ndocuments, nlines, dpTestTemplate.connection);

		const iterationTimer = await dpTestTemplate.performFlow({connection: dpTestTemplate.connection});
		...
		const {newNumberOfDocuments, newNumberOfLines} = getDocumentsAndLinesNumberForIteration(dpTestTemplate, ndocuments, nlines);

		if (newNumberOfDocuments > 0 && newNumberOfLines > 0 && iterationTimer.getTime() > 0)
			resultsAccumulator = await gatherResults(dpTestTemplate, cloneDeep(resultsAccumulator), newNumberOfDocuments, newNumberOfLines);
...
const getDocumentsAndLinesNumberForIteration = (dpTestTemplate: BulkDocumentProcessTestTemplate, currentNumberOfDocuments: number, currentNumberOfLines: number) => {
		let newNumberOfDocuments: number;
		let newNumberOfLines: number;

		if (doNumberLinesPlusIncrementReachMaximum(dpTestTemplate, currentNumberOfLines)) {
			if (doNumberDocumentsPlusIncrementReachMaximum(dpTestTemplate, currentNumberOfDocuments)) {
				newNumberOfDocuments = -1;
				newNumberOfLines = -1;
			} else {
				newNumberOfLines = dpTestTemplate.nLinesInitial;
				newNumberOfDocuments = currentNumberOfDocuments + dpTestTemplate.nDocumentsPerIteration;
			}
		} else {
...
const doNumberDocumentsPlusIncrementReachMaximum = (dpTestTemplate: BulkDocumentProcessTestTemplate, currentNumberOfDocuments: number): boolean => {
	if (dpTestTemplate.nDocumentsMaximum && ((currentNumberOfDocuments + dpTestTemplate.nDocumentsPerIteration) > dpTestTemplate.nDocumentsMaximum)) {
		return true;
	}
	return false;
};

const doNumberLinesPlusIncrementReachMaximum = (dpTestTemplate: BulkDocumentProcessTestTemplate, currentNumberOfLines: number): boolean => {
	if ((currentNumberOfLines + dpTestTemplate.nLinesIteration) > dpTestTemplate.nLinesMaximum) {
		return true;
	}
	return false;
};

const addResult = (processResults: BulkDocumentProcessResult[], result: BulkDocumentProcessResult) => {
	const newProcessResults: BulkDocumentProcessResult[] = cloneDeep(processResults);
	newProcessResults.push(result);

	return newProcessResults;
};

const onError = (dpTestTemplate: BulkDocumentProcessTestTemplate, e: TestPerformanceErrorI) => {
	return Timer.createFromException(`${dpTestTemplate.action}: error`, e);
};

const saveResults = async (dpTestTemplate: BulkDocumentProcessTestTemplate, results: BulkDocumentProcessResult[] ) => {
	console.log(`Saving results...`);

	let testResults: TestResultI[];

	testResults = results.map( (result: BulkDocumentProcessResult) => {
	...
	const orgContext: OrgContext = await getOrgContext(dpTestTemplate.connection);
