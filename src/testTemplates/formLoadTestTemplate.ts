...
export class FormLoadTestTemplate {
  ...
  public async before() {
    ...
    this._connection = await connectToSalesforceOrg(
      getSalesforceAuthInfoFromEnvVars()
    );
    ...
    while (!browserOpened && attempt < 3) {
      try {
        ...
        context = this._incognitoRequired
          ? await this._browser.createBrowserContext()
          : this._browser.browserContexts()[0];
  ...
  public async it(
    callback: () => void,
    configuration: BenchmarkerPage | string
  ) {
    let pageName: string;
    ...
    if (typeof configuration === 'string') {
      ...
      ({ pageName, lighthouseEnabled } = configuration);
    }
    ...
    try {
      ...
      if (
        lighthouseFeatureEnabled &&
        lighthouseEnabled &&
        !this._incognitoRequired
      ) {
        if (getLighthouseHTMLReport()) {
          await getLighthouseMetricsAndSaveFile(
            'lighthouse-report',
            `${pageName}_${moment().format()}.html`,
            this._page
          );
        } else {
          const lighthouseMetrics: ProcessedLighthouseMetrics[] =
            await getLighthouseMetrics(this._page);
          lighthouseSpeedIndex = Math.round(
            lighthouseMetrics.find(
              metricItem => metricItem.metric === 'speed-index'
            )!.value
          );
          lighthouseTimeToInteractive = Math.round(
            lighthouseMetrics.find(
              metricItem => metricItem.metric === 'interactive'
            )!.value
          );
        }
      }
  ...
  public async after() {
    ...
    const testResults: TestResultOutput[] = this._metricsArray.map(
      (metricsItem: MetricsSuite) => {
        const testResult: TestResultOutput = {
          ...
          lighthouseTimeToInteractive:
            metricsItem.lighthouseMetrics.timeToInteractive,
    ...
    try {
      const orgContext = await getOrgContext(this._connection);
