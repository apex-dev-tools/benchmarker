...
describe('shared/env/index', () => {
	...
	describe ('getPuppeteerLaunchOptions', () => {
		...
		it('returns debug-friendly config in debug mode', () => {
			...
			expect(launchOptions).to.eql({
				args: [`--proxy-server='direct://'`, '--proxy-bypass-list=*', '--no-sandbox', '--disable-setuid-sandbox', '--window-size=1920,1080', '--disable-dev-shm-usage'],
				headless: false,
				slowMo: 100,
				timeout: 0
			});
	...
	describe("getAppLauncherTabSelector", () => {
		it("returns APP_LAUNCHER_ALL_TABS_SELECTOR given value", () => {
			// Given
			process.env.APP_LAUNCHER_ALL_TABS_SELECTOR = "Test tab=$tabName, attribute=$tabAttribute";
