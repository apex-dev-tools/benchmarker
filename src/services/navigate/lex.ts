...
import {
  getAppLauncherSelector,
  getAppLanucherAllTabsSelector,
  getAppLauncherTabSelector,
} from '../../shared/env';
...
async function goToFrontdoor(
  ...
  const response = await page.goto(frontdoorUrl, {
...
export async function goToAllTabsTab(
  config: INavigateConfig,
  tabName: string,
  tabAttribute?: string
  ...
  const appLauncherButton = await searchSelector(
    page,
    getAppLauncherSelector(),
    false,
    60000
  ...
  const appLauncherViewAll = await searchSelector(
    page,
    getAppLanucherAllTabsSelector(),
    false,
    60000
  ...
  const element = await searchSelector(
    page,
    getAppLauncherTabSelector(tabName, tabAttribute),
    false,
    60000
  );
...
async function getVfFrame(config: INavigateConfig): Promise<Frame> {
  ...
  if (config.title) {
    await page.waitForSelector(
      `iframe[name*="${vfFramePrefix}"][title*="${config.title}"]`,
    ...
    await page.waitForSelector(`iframe[name*="${vfFramePrefix}"]`, {
      timeout: 60000,
      visible: true,
    });
    ...
    if (iframeFound)
      if (config.pageDomain)
        iframeFound = x
          .url()
          .toLocaleLowerCase()
          .includes(config.pageDomain.toLocaleLowerCase());
