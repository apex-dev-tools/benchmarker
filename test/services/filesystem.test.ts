...
describe('/src/services/filesystem', () => {
  ...
  describe('saveFileIntoDir', () => {
    it('Directory does not exist', async () => {
      // Given
      sinon.stub(promises, 'stat').rejects();
      const writeFile = sinon.stub(promises, 'writeFile').resolves();
      const mkdir = sinon.stub(promises, 'mkdir').resolves();
      ...
      expect(writeFile).to.have.been.calledOnceWith(
        './myDir/myFile',
        'content'
      );
      expect(result);
    ...
    it('Directory exists', async () => {
      // Given
      sinon.stub(promises, 'stat').resolves();
      const writeFile = sinon.stub(promises, 'writeFile').resolves();
      const mkdir = sinon.stub(promises, 'mkdir').resolves();
    ...
    it('FileWrite fails', async () => {
      // Given
      sinon.stub(promises, 'stat').resolves();
      const writeFile = sinon
        .stub(promises, 'writeFile')
  ...
  describe('csv2Array', () => {
    it('reads csv file and returns an array with a given number of items', () => {
      ...
      const generatedRows = generateRecordsWithCSVData(
        'path',
        ['EID1', 'EID2'],
        10
      );
      ...
      expect(generatedRows[0]).to.be.deep.equal({
        header1: 'valueA1',
        header2: 'valueA2',
        ExternalId: 'EID1',
      });
      expect(generatedRows[1]).to.be.deep.equal({
        header1: 'valueB1',
        header2: 'valueB2',
        ExternalId: 'EID1',
      });
      expect(generatedRows[10]).to.be.deep.equal({
        header1: 'valueA1',
        header2: 'valueA2',
        ExternalId: 'EID2',
      });
      expect(generatedRows[11]).to.be.deep.equal({
        header1: 'valueB1',
        header2: 'valueB2',
        ExternalId: 'EID2',
      });
    ...
    it('reads csv file and returns an array with default number of items', () => {
      ...
      const generatedRows = generateRecordsWithCSVData('path', [
        'EID1',
        'EID2',
      ]);
  ...
  describe('readFile', () => {
    it('returns file contents', async () => {
      sinon.stub(fs, 'readFile').yields(undefined, 'file content');
    ...
    it('throws an error when file is not found', async () => {
      sinon.stub(fs, 'readFile').yields('file not found', undefined);
      try {
