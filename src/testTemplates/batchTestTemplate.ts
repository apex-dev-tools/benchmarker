...
export class BatchProcessTestTemplate {
   ...
   * @param connection object to handle the connection to a Salesforce Org
   */
  public checkProcessCanBeLaunched: (
    connection: SalesforceConnection
  ) => Promise<boolean>;
  ...
  public checkProcessStatus: (
    batchProcessParams: BatchProcessCheckerParams
  ) => Promise<BatchProcessStatusResult>;

  public readonly batchStatusCheckingIntervalTime: number =
...
export namespace BatchProcess {
  ...
  export const performTestFlow = async (
    ...
    const runProcess = await batchTestTemplate.checkProcessCanBeLaunched(
      batchTestTemplate.connection
    );
    console.log(`${batchTestTemplate.action} can be launched? ${runProcess}`);
    if (runProcess) await gatherResults(batchTestTemplate);
  };
}
...
const gatherResults = async (
  ...
  try {
    const dataCreationResult = await batchTestTemplate.dataCreation(
      ndocuments,
      nlines,
      batchTestTemplate.connection
    );
    ...
    if (dataCreationResult) {
      processStartResult = await batchTestTemplate.startProcess({
        connection: batchTestTemplate.connection,
        action: batchTestTemplate.action,
      });
    } else {
      console.error(
        `${batchTestTemplate.action} data was not created correctly, the test execution will not continue`
      );
    }
  } catch (e) {
    console.error(
      `Exception on ${batchTestTemplate.action}, ${e.message}, error info: ${e.exception}`
    );
  ...
  if (
    processStartResult &&
    (processStartResult.apexJobId || processStartResult.customParameter)
  ) {
    if (batchTestTemplate.batchStatusCheckingIntervalTime > 0)
      // Mainly for testing
      await monitorAsyncProcessStatus(batchTestTemplate, {
        connection: batchTestTemplate.connection,
        apexJobId: processStartResult.apexJobId,
        customParameter: processStartResult.customParameter,
        action: batchTestTemplate.action,
        processStatusResult: {},
      })
        .then(processResult => {
          const lastTimer = processResult.timer
            ? processResult.timer
            : onError(batchTestTemplate, processResult.error!);
        ...
        .catch(e =>
          console.error(
            `Exception happened during ${batchTestTemplate.action} test execution: ${e}`
          )
        )
        .finally(async () => {
          console.log(
            `Deleting data on ${
              batchTestTemplate.action
            }, timeStamp: ${new Date()}`
          );
          return await batchTestTemplate.dataDeletion(
            batchTestTemplate.connection
          );
        });
  } else {
    console.warn(
      `Unable to get ApexJobId for ${batchTestTemplate.action}, async process will not be monitored`
    );
    console.log(
      `Deleting data on ${batchTestTemplate.action}, timeStamp: ${new Date()}`
    );
    await batchTestTemplate.dataDeletion(batchTestTemplate.connection);
  }
};
...
const saveResults = async (
  batchTestTemplate: BatchProcessTestTemplate,
  results: BatchProcessResults[]
) => {
  console.log(
    `Saving results ${batchTestTemplate.action}, timeStamp: ${new Date()}`
  );

  const testResults: TestResultOutput[] = results.map(
    (result: BatchProcessResults) => {
  ...
  const orgContext = await getOrgContext(batchTestTemplate.connection);

  await reportResults(testResults, orgContext);
};

const defaultCheckProcessStatus = async (
  batchProcessParams: BatchProcessParams
): Promise<BatchProcessStatusResult> => {
  console.log(
    `Default BatchTestTemplate process status checker: Checking Job Status for apexJobId ${batchProcessParams.apexJobId}`
  );
  const response = await query(
    batchProcessParams.connection,
    `select status, CompletedDate, CreatedDate, NumberOfErrors, ExtendedStatus from AsyncApexJob WHERE id = '${batchProcessParams.apexJobId}'`
  );
  ...
  if (response.records.length > 0) {
    ...
    console.info(
      `Checking ${batchProcessParams.action} job status, currently it is: ${
        apexJobRecord.Status
      }, NumberOfErrors: ${apexJobRecord.NumberOfErrors}, PID: ${
        process.pid
      } ,timeStamp: ${new Date()}`
    );
    ...
    switch (apexJobRecord.Status) {
      ...
      case 'Failed':
        statusCheckResult.failed = true;
        statusCheckResult.errorMessage =
          apexJobRecord.ExtendedStatus || 'Job failed';
        break;
      case 'Completed':
        const timer: Timer = Timer.create([
          { label: batchProcessParams.action },
        ])[0];
        ...
        console.info(
          `${batchProcessParams.action} lasted ${diff / 1000} seconds`
        );
        break;
    }
  }
...
const monitorAsyncProcessStatus = async (
  batchTestTemplate: BatchProcessTestTemplate,
  batchParams: BatchProcessCheckerParams
): Promise<{ timer?: Timer; error?: TestPerformanceErrorI }> => {
  await delay(batchTestTemplate.batchStatusCheckingIntervalTime);

  const result: BatchProcessStatusResult =
    await batchTestTemplate.checkProcessStatus({
      connection: batchParams.connection,
      action: batchParams.action,
      apexJobId: batchParams.apexJobId,
      customParameter: batchParams.customParameter,
      processStatusResult: batchParams.processStatusResult,
    });
  ...
  if (result.failed) {
    return Promise.resolve({
      error: {
        message:
          result.errorMessage ||
          'Error produced in batch but no info was provided',
      },
    });
  }
  ...
  return Promise.resolve(
    await monitorAsyncProcessStatus(batchTestTemplate, newBatchParams)
  );
};
