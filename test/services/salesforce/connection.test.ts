...
describe('salesforce connection module', () => {
	...
	describe('connectToSalesforceOrg', () => {
		it('connect to non-scratch org', async () => {
			...
			const jsforceStub = stub(jsforce);

			const connectionConstructor = jsforceStub.Connection
				.returns({
			...
			const sfdxCoreConnectionCreate = stub(SalesforceConnection, 'create').resolves({
				instanceUrl: undefined
			});

			const connection = await connectToSalesforceOrg({ username: 'username', password: 'passwordtoken', loginUrl: 'loginUrl' });

			expect(connectionConstructor).to.have.been.calledOnceWith({ loginUrl: 'loginUrl' });
			expect(login.args[0]).to.be.have.members(['username', 'passwordtoken']);
			expect(authInfoCreate).to.have.been.calledOnceWith({ username: 'accessToken' });
		...
		it('connect to scratch org', async () => {
			...
			const connection = await connectToSalesforceOrg({ username: 'username', isSFDX: true });

			expect(refreshAuthStub).to.have.been.called;
			expect(orgCreate).to.have.been.calledWith({ aliasOrUsername: 'username' });
		...
		it('connect to scratch org but fails', async () => {
			...
			try {
				...
				expect(e.message).to.contains('fake error');
			}
		});
	});
