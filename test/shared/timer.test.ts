...
describe('src/scenario/shared/timer', () => {
	...
	it('should create several timers', async () => {
		const [ testTimer1, testTimer2 ] = Timer.create([{label: 'Test1'}, {label: 'Test2'}]);
	...
	it('should returns the difference between to times', async () => {
		...
		let momentStub =  stub(moment, 'now').returns(startTime);
		testTimer.start();

		momentStub.restore();

		const endTime = moment('2015-02-02 11:40:45', 'YYYY-MM-DD HH:mm:ss');
		momentStub =  stub(moment, 'now').returns(endTime);
		testTimer.end();
	...
	it('should create a timer from a exception', async () => {
		const testTimer: Timer = Timer.createFromException('Test Page Description', {message: 'Test message', exception: 'Test exception produced in test'});

		expect(testTimer.error).to.eql('Test exception produced in test. Test message');
		expect(testTimer.getTime()).to.eql(-1);
	});

	it('should create a timer from a message', async () => {
		const testTimer: Timer = Timer.createFromException('Test Page Description', {message: 'Test message', exception: undefined});
