...
import { TransactionTestTemplate, TransactionProcess, FlowStep, TestFlowOutput } from '../../src/testTemplates/transactionTestTemplate';
import * as utils from '../../src/services/salesforce/utils';

chai.use(sinonChai);

describe('src/test-templates/transaction-test-template', () => {
	...
	describe('TransactionProcess.build', () => {
		it('returns a TransactionTestTemplate instance with a connection ready to be used', async () => {
			...
			// When
			const test: TransactionTestTemplate = await TransactionProcess.build(MOCKPRODUCT_FULLNAME);
	...
	describe('TransactionProcess.executeTestStep', () => {
		it('resolves and save results', async () => {
			...
			// Then
			expect(test.flowStepsResults[0].testStepDescription.action).to.be.eq('Mock action');
			expect(test.flowStepsResults[0].testStepDescription.flowName).to.be.eq('Mock flow name');
