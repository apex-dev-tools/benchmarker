...
import { BULK_API_DEFAULT_CHUNK_SIZE, BULK_API_INSERT_OPERATION, ERROR_DUPLICATE_VALUE } from '../../shared/constants';
 ...
 * ```
 */
export async function deleteRecords(connection: SalesforceConnection, object: string, whereField: string, whereValue: string) {
 ...
 * ```
 */
export async function deleteGenericRecordsByExternalIDs(connection: SalesforceConnection, externalIDs: string[], objectApiName: string, externalIdApiName: string, chunkSize: number) {
	try {
		const promisesArray: Array<Promise<any>> = chunk(externalIDs, chunkSize).map((externalIdsChunk: string[]) => {
			return deleteBulkRecords(connection, replaceNamespace(objectApiName), `${replaceNamespace(externalIdApiName)} IN ('${externalIdsChunk.join('\',\'')}')`);
		});
 ...
 * ```
 */
export async function createSobjectRecord(connection: SalesforceConnection, object: string, fields: {}) {
 ...
 * ```
 */
export async function getSobjectRecord(connection: SalesforceConnection, object: string, fields: {}) {
 ...
 * ```
 */
export async function getSobjectRecords(connection: SalesforceConnection, object: string, fields: {}) {
 ...
 * ```
 */
export async function bulkInsert(connection: SalesforceConnection, properties: {
	nItems: number,
	...
	return new Promise(async (resolve, reject) => {
		...
		if (properties.parentExternalIds) {
			objectRecordsArray = generateRecordsWithCSVData(properties.csvPath, properties.parentExternalIds, properties.nItems);
		} else {
			generatedExternalIDs = generateExternalIds(properties.nItems, properties.externalIdPrefix);
			objectRecordsArray = generateRecordsWithCSVData(properties.csvPath, generatedExternalIDs);
		}
		console.info(`Starting data creation for ${properties.nItems} ${properties.objectApiName}`);
		try {
			const timer: Timer = Timer.create([{label: 'Measure-data-creation'}])[0];
			timer.start();
			const batchResults = await insertDataWithBatches(connection, properties.objectApiName, BULK_API_INSERT_OPERATION, objectRecordsArray, chunkSize);
			timer.end();
			if (batchResults.some( batchResult => batchResult.failed)) {
				const errorResult = (
					batchResults
						.filter( batchResult => batchResult.failed)
						.reduce( (accumulator: string, element: {failed: boolean, errors: string[]}) => {
				...
				console.info(`Insertion operation for ${properties.nItems} ${properties.objectApiName} ended, time consumed: ${timer.getTime() / 1000} sec`);
				resolve(generatedExternalIDs!);
			}
		} catch (e) {
			reject(`${properties.objectApiName} error occurred during creation, message = ${e.message}`);
		}
	});
}
/**
...
async function deleteBulkRecords(connection: SalesforceConnection, object: string, whereCondicion: string) {
	try {
		await sobject(connection, object)
			.select()
