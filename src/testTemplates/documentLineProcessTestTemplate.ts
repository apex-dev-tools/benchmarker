...
import { DEFAULT_NUMERIC_VALUE, DOCUMENT_LINE_PROCESS } from '../shared/constants';
...
export namespace DocumentLineProcess {
	 ...
	 */
	export const build = async (product: string, linesInitial: number, linesPerIteration: number, linesMaximum: number, action: string, testType: string = DOCUMENT_LINE_PROCESS): Promise<DocumentLineProcessTestTemplate> => {
	 ...
	 */
	export const performTestFlow = async (dlpTestTemplate: DocumentLineProcessTestTemplate) => {
		try {
...
const gatherResults = async (dlpTestTemplate: DocumentLineProcessTestTemplate, resultsAccumulator: DocumentLinesProcessResults[] = [], iteration: number = 1, lines: number = dlpTestTemplate.linesInitial) => {
	try {
		await dlpTestTemplate.dataCreation(lines, dlpTestTemplate.connection);
		const iterationTimer = await dlpTestTemplate.performFlow({connection: dlpTestTemplate.connection});
		...
		if (newLines > 0 && iterationTimer.getTime() > 0)
			resultsAccumulator = await gatherResults(dlpTestTemplate, cloneDeep(resultsAccumulator), iteration++, newLines);
...
const getLinesForIteration = (dlpTestTemplate: DocumentLineProcessTestTemplate, iteration: number) => {
		const estimatedLines = dlpTestTemplate.linesInitial + (dlpTestTemplate.linesPerIteration * iteration);
		if (dlpTestTemplate.linesMaximum && dlpTestTemplate.linesMaximum > DEFAULT_NUMERIC_VALUE && estimatedLines > dlpTestTemplate.linesMaximum )
			return -1;

		return estimatedLines;
};

const addResult = (processResults: DocumentLinesProcessResults[], result: DocumentLinesProcessResults) => {
	const newProcessResults: DocumentLinesProcessResults[] = cloneDeep(processResults);
	newProcessResults.push(result);

	return newProcessResults;
};

const onError = (dlpTestTemplate: DocumentLineProcessTestTemplate, e: TestPerformanceErrorI) => {
	return Timer.createFromException(`${dlpTestTemplate.action}: error`, e);
};

const saveResults = async (dlpTestTemplate: DocumentLineProcessTestTemplate, results: DocumentLinesProcessResults[] ) => {
	console.log(`Saving results ${dlpTestTemplate.action}`);
	let testResults: TestResultI[];

	testResults = results.map( (result: DocumentLinesProcessResults) => {
	...
	const orgContext: OrgContext = await getOrgContext(dlpTestTemplate.connection);
