...
export class Timer {
	 ...
	 */
	public static create(timerDefinitions: Array<{label: string, targetValue?: number}>): Timer[] {
		const timers = timerDefinitions.map(element => new Timer(element.label, element.targetValue));
		return timers;
	}
	 ...
	 */
	public static createFromException(description: string, e: TestPerformanceErrorI): Timer {
