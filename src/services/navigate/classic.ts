...
import { INavigate, INavigateConfig } from '../navigate';

async function goToFrontdoor(
  config: INavigateConfig,
  frontdoorUrl: string
): Promise<HTTPResponse | null> {
  const page = config.page;
  const response = page.goto(frontdoorUrl, {
...
async function goToAllTabsTab(
  ...
  const element = await page.waitForSelector(
    `xpath///a[contains(@class, "listRelatedObject")${xpathPartTabName}]`
  );
...
async function getVfFrame(config: INavigateConfig): Promise<Frame> {
  const page = config.page;
  return Promise.resolve(page.mainFrame());
}

async function homeIsLoaded(
  config: INavigateConfig
