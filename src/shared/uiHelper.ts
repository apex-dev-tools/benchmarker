...
/** @ignore */
export function retryMaxAttempsWithDelay<T>(workToDo: () => Promise<T>, delayMillis: number, maxAttempts: number, attempts: number = 0): Promise<T> {
		...
		try {
			...
			if (attempts < maxAttempts) {
				...
				try {
					...
					resolve(await common.retryMaxAttempsWithDelay(workToDo, delayMillis, maxAttempts, attempts));
				} catch (err) {
					reject(err);
				}
			} else {
				reject( {message: 'Max attempts reached', exception: err} as TestPerformanceErrorI);
			}
		}
	});
}
 ...
 * ```
 */
export async function searchSelector(frame: Page, selector: string, visible: boolean, timeout: number) {
	const selectorToWait = () => new Promise<ElementHandle>(async (resolve, reject) => {
		try {
			const element = await frame.waitForSelector(selector, {visible , timeout});
			if (element) {
				resolve(element);
			} else {
...
export async function switchView(page: Page, timeoutToWait: number) {
	await page.waitForSelector('div div div div div div[class="slds-spinner slds-spinner--medium slds-spinner--brand"]', {visible:  true, timeout: timeoutToWait});
	await page.waitForSelector('div div div div div div[class="slds-spinner slds-spinner--medium slds-spinner--brand"]', {visible: false, timeout: timeoutToWait});
	await delay(3000);
}
 ...
 * ```
 */
export async function retryWaitToHomePage(frontdoorUrl: string, navigator: INavigate, page: Page) {
	await retryMaxAttempsWithDelay(async () => await waitToFrontdoor(frontdoorUrl, page, navigator), 3000, 3);
}
 ...
 * ```
 */
export async function retryWaitToPageTabButton(test: FormLoadTestTemplate, page: Page, pageName: string, pageAttribute?: string) {
	return await retryMaxAttempsWithDelay(async () => await test.navigator.goToAllTabsTab({ page }, pageName, pageAttribute), 3000, 3);
}
...
async function waitToFrontdoor(frontdoorUrl: string, page: Page, navigator: INavigate) {
