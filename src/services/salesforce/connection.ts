...
export const connectToSalesforceOrg = async (authInfoWrapper: SalesforceAuthInfo): Promise<SalesforceConnection> => {
	...
	try {
		...
		throw new Error(`Exception happened in the Salesforce authentication process. Exception message: ${e}`);
	}
 ...
 */
export const getSalesforceAuthInfoFromEnvVars = (): SalesforceAuthInfo => getSfdxUsername() ? { username: getSfdxUsername(), isSFDX: true } : {username: getSalesforceUsername(), password: getSalesforcePassword() + getSalesforceToken(), loginUrl: getSalesforceUrlLogin()};

const connectToSFDXOrg = async (authInfoWrapper: SalesforceAuthInfo): Promise<SalesforceConnection> => {
	const sfOrg = await Org.create({ aliasOrUsername: authInfoWrapper.username });
	await sfOrg.refreshAuth();
	return sfOrg.getConnection();
};

const connectToStandardOrg = async (authInfoWrapper: SalesforceAuthInfo): Promise<SalesforceConnection> => {
	let connection: SalesforceConnection;
	const jsForceConnection = new jsforce.Connection({loginUrl: authInfoWrapper.loginUrl!});
